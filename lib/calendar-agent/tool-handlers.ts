...
export interface CalendarToolHandlers {
  ...
  }) => Promise<{ success: boolean; summary?: { totalEvents: number; events: any[]; stats?: { upcoming: number; past: number; allDay: number } }; error?: string }>;
}

export const createCalendarToolHandlers = (calendarStore: any): CalendarToolHandlers => ({
  ...
  updateEvent: async (params) => {
    try {
      const existingEvent = calendarStore.events.find((e: Event) => e.id === params.eventId);
      if (!existingEvent) {
  ...
  getCalendarSummary: async (params) => {
    try {
      ...
      const summary: { totalEvents: number; events: any[]; stats?: { upcoming: number; past: number; allDay: number } } = {
      ...
      if (params.includeStats) {
        const now = new Date();
        const upcomingEvents = events.filter((event: Event) => new Date(event.startDate) > now);
        const pastEvents = events.filter((event: Event) => new Date(event.endDate) < now);
