...
http.route({
  ...
  handler: action(async (ctx, request) => {
    const { clerkUserId, googleUserId, email, accessToken, refreshToken, expiresAt, scopes } = await request.json()
    
    if (!clerkUserId || !googleUserId || !email || !accessToken || !expiresAt || !scopes) {
