...
import { DemoEventCard } from '@/components/demo/event-card';
...
const dummyEvents: Event[] = [
  {
    id: '1',
    title: 'Team Meeting',
    startDate: new Date(2024, 7, 12, 9, 0),
    endDate: new Date(2024, 7, 12, 10, 0),
    type: 'event',
    color: 'blue',
  ...
  },
  {
    ...
    startDate: new Date(2024, 7, 12, 12, 0),
    endDate: new Date(2024, 7, 12, 13, 0),
    type: 'event',
    color: 'green',
  ...
  },
  {
    id: '4',
    title: 'Coffee Break',
    startDate: new Date(2024, 7, 14, 10, 30),
    endDate: new Date(2024, 7, 14, 11, 0),
    type: 'event',
    color: 'orange',
  ...
  },
  {
    id: '5',
    title: 'All Day Event',
    startDate: new Date(2024, 7, 15, 0, 0),
    endDate: new Date(2024, 7, 15, 23, 59),
    isAllDay: true,
    type: 'event',
    color: 'red',
    account: 'jane.smith@outlook.com',
  },
];
...
export default function WeeklyView() {
  ...
  function handleAddEventWeek(dayIndex: number, detailedHour: string) {
    console.log('Add event for day', dayIndex, 'at', detailedHour);
  }
  ...
  return (
    <div className="flex flex-col gap-4">
      <AnimatePresence custom={direction} initial={false} mode="wait">
         ...
         >
          <div className="col-span-1 flex items-center justify-center border-white/5 border-r border-b bg-neutral-900 py-2">
          ...
          <div className="relative col-span-8 flex flex-col">
            <div
              className="sticky top-0 z-40 grid flex-grow gap-0 border-white/5 border-b bg-neutral-900 backdrop-blur"
              style={{
            ...
            >
              {daysOfWeek.map((day, idx) => (
                <div className="group relative flex flex-col" key={idx}>
                  <div className="flex flex-grow items-center justify-center border-white/5 border-r bg-neutral-900 py-2">
                    <div className="text-center">
                      <div
                                                 className="font-medium text-white/60 text-xs"
                      >
            ...
            <div
              className="grid gap-0 border-white/5 border-b bg-neutral-900/50"
              style={{
                gridTemplateColumns: colWidth.map((w) => `${w}fr`).join(' '),
                transition: isResizing
                  ? 'none'
            ...
            >
              {daysOfWeek.map((day, dayIndex) => {
                ...
                return (
                    <div
                      className="relative min-h-[32px] border-white/5 border-r p-1"
                        key={`allday-${dayIndex}`}
                    >
          ...
          <div
            ...
            ref={hoursColumnRef}
          >
            <div className="col-span-1 border-white/5 border-r bg-neutral-900">
              {hours.map((hour, index) => (
                <motion.div
                  className="flex h-[64px] cursor-pointer items-start justify-center border-white/5 border-b px-3 py-2 text-left text-white/60 text-xs"
                  key={`hour-${index}`}
            ...
            >
              {detailedHour && (
                <div
                  className="pointer-events-none absolute left-0 z-50 flex h-[1px] w-full rounded-full bg-white/20"
                ...
                return (
                  <ContextMenu key={`day-${dayIndex}`}>
                    <ContextMenuTrigger asChild>
                        <div
                          className="relative z-20 col-span-1 overflow-hidden border-white/5 border-r border-b text-center text-white/60 text-sm transition duration-300"
                      ...
                      >
                        <AnimatePresence initial={false}>
                          {visibleEvents?.map((event, eventIndex) => {
                           ...
                           <div
                             key={`hour-${hourIndex}`}
                             className="relative z-10 col-span-1 h-[64px] border-white/5 border-r border-b text-center text-white/60 text-sm transition duration-300"
                           />
                         ))}
                      </div>
                    </ContextMenuTrigger>
